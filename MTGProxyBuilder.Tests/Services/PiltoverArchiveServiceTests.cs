...
public class PiltoverArchiveServiceTests
{
    ...
    [Fact]
    public void AllCards_EnumeratesEverySection()
    {
        static RiftboundDeckCard Entry(string id) => new()
        {
            Card = new RiftboundCard { Id = id, Name = id }
        };

        var deck = new RiftboundDeck
        {
            Legend = Entry("legend"),
            Champions = { Entry("champ") },
            Battlefields = { Entry("field") },
            Runes = { Entry("rune") },
            Maindeck = { Entry("main") },
            Sideboard = { Entry("side") },
            Bench = { Entry("bench") },
        };

        Assert.Equal(new[] { "legend", "champ", "field", "rune", "main", "side", "bench" },
            deck.AllCards().Select(c => c.Card.Id));
    }
}
