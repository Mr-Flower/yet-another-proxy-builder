...
{
    /// <summary>
    /// Fetches Riftbound decks from piltoverarchive.com by extracting the deck JSON embedded in the
    /// page's React Server Components payload, and downloads card art via the shared image cache.
    /// Ported from upstream (anubisascends/mtg-proxy-builder).
    /// </summary>
    public class PiltoverArchiveService
    {
        ...
        {
            // Browser-like UA: the site serves a plain Next.js page and has no API; the default
            // bot-looking UA can get filtered by its CDN.
