...
public partial class ColorPickerWindow : Window
{
    ...
    private void BuildSwatches()
    {
        foreach (var hex in PresetColors)
        {
            ...
            {
                Width = 30, Height = 30, Margin = new Thickness(2),
                CornerRadius = new CornerRadius(3), Cursor = new Avalonia.Input.Cursor(Avalonia.Input.StandardCursorType.Hand),
                BorderThickness = new Thickness(2), BorderBrush = Brushes.Transparent
            };

            try { border.Background = new SolidColorBrush(Color.Parse(hex)); }
            catch { border.Background = Brushes.Gray; }

            string capturedHex = hex;
            border.PointerReleased += (_, _) => SetFromHex(capturedHex);
    ...
    private void UpdatePreview()
    {
        ...
        foreach (var child in SwatchPanel.Children)
        {
            ...
            {
                bd.BorderBrush = ToolTip.GetTip(bd)?.ToString() == SelectedHexColor
